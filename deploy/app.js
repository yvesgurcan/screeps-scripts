...
function start() {
    ...
    console.log(`${_.size(data.modules)} files found.`);
...
function safenFilename(rawName) {
    if (
        rawName !== undefined &&
        rawName.length > 3 &&
        rawName.substring(rawName.length - 3, rawName.length) === '.js'
    ) {
...
function setup() {
    ...
    if (path) {
        fs.writeFileSync(
            path,
            fs.readFileSync(__dirname + '/config.js.sample')
        );
    } else {
        ...
        console.log();
        console.log(
            'Or set the SCREEPSDEPLOY_CONFIG_PATH environment variable to point to a valid config file.'
        );
    }
}
...
function getConfigPaths() {
    ...
    if (process.platform === 'linux' || process.platform === 'darwin') {
        ...
        paths.push(create);
        paths.push(`/etc/${appname}/config.json`);
    }
    ...
    if (process.platform === 'win32') {
        ...
        if (!fs.existsSync(path.join(dir, 'config.json'))) {
            fs.writeFileSync(
                ...
                fs.readFileSync(path.join(__dirname, 'config.js.sample'))
            );
        }
